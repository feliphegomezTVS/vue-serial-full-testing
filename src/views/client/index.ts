...
export default ({
    ...
    methods: {
        ...
        /* CAN Bus */ 
        canFilterInput(CANBUSMessage: String) {
            try {
                if (CANBUSMessage.indexOf('ERROR') === -1) {
                    this.canb.CANCounterGUI++;
                    if (CANBUSMessage.slice(4, 5) !== "2") {
                        this.canb.CANMessageGUI = CANBUSMessage;
                    }
                    // Validando si es un msg de importancia
                    if (this.canb.info.a.test(CANBUSMessage)) {
                        this.canb.pauseIP = false;
                        
                        this.canb.CANActionGUI = this.canb.dinf.k; // Revisar comando

                        if (this.canb.info.k.test(CANBUSMessage)) {
                            this.canb.recallLastFlashBlock = false;
                            this.canb.pauseFlashBlockArray = true;
                            this.canbarrayLastFlashBlock = [];
                        }
                        if (this.info.e.test(CANBUSMessage) && (this.pauseFlashBlockArray === true)) {
                        this.canb.pauseIPinstant = false;
                        this.canb.pauseIP = true;
                ...
                try {
                    ...
                    for (;;) {
                        ...
                        if (value) {
                            ...
                            let value2 = self.canFilterInput(value);
